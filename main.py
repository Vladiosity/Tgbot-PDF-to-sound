...
def on_help(message):
    bot.send_message(message.chat.id, 'Вот список того, что я могу: отправьте мне файл в формате .pdf или .txt, и я скину вам mp3 с содержимым '
                                      ...
                                      'Также вы можете отправить мне текст начинающийся на "Сделай пдф:" и я пришлю вам файл pdf с вашим текстом(пока только на английском).' 
...
def start_msg(message):
    bot.send_message(message.chat.id, 'Привет! Я - ботяра, который может "прочитать" глупым роботским голосом текcт, '
                                      'PDF файл или текcтовый файл в формате mp3 или же сделать пдф из текста.'
                                      'Я создан только для этого, так что сразу отправляйте мне файл, '
